...
public class ToDoAdapter extends RecyclerView.Adapter<ToDoCardHolder>{
    ...
    public void onBindViewHolder(@NonNull ToDoCardHolder holder, int position) {
        ...
        holder.edit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent i = new Intent((Activity)context, EditTodo.class);
                i.putExtra("currentId",todos.get(position).getId());
                i.putExtra("currentText",todos.get(position).getText());
                i.putExtra("currentType",todos.get(position).getType());
